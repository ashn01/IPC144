	...
	do
	{
		...
		case 2: // Withdraw funds
			...
			if (isFound != -1)
			{
				...
				scanf("%lf", &withdraw);
				if (acct[isFound].balance > withdraw)
				{
					acct[isFound].balance = balanceDown(acct[isFound].balance, withdraw);
					printf("Current balance is : %.2lf\n\n", acct[isFound].balance);
				}
				else
		...
		case 4: // Apply Service Charges
			...
			for (i = 0; i < nosClients; i++)
			{
				...
				acct[i].balance = balanceDown(acct[i].balance, serviceCharge);
				printf("%8d %11.2lf %19.2lf\n", acct[i].accNumber, acct[i].balance, serviceCharge);
			}
